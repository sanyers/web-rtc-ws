...
sendText.onclick = () => {
  const text = document.getElementById("textPage").value;
  pcList.forEach((element) => {
    element.pc._channel.send(text);
  });
};
...
function createWebRTC(userName) {
  ...
  const channel = pc.createDataChannel("aaa", {
    ordered: true,
    maxRetransmits: 30,
  });

  channel.addEventListener("message", (ev) => {
    msgDiv.innerHTML += `<p>${ev.data}</p>`;
  });

  pc.addEventListener("datachannel", (ev) => {
    console.log("111");
    pc._channel = ev.channel;
  });
